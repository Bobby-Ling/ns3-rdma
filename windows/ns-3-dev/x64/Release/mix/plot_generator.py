...
def plot_metrics(time_slots, bandwidths, intervals, timestamps, show_plot=True):
    """生成带宽和波动率随时间变化的图"""
    ...
    # 创建图表
    plt.figure(figsize=(12, 12))  # Increased figure height to accommodate new subplot
    ...
    # 绘制时间差值图（使用前一个时间戳作为x轴）
    plt.plot(np.array(unique_times[:-1]) * 1000, time_diffs * 1000, 'g-', linewidth=1, label='Time Difference')
    plt.title('Time Differences between Consecutive Unique Timestamps')
    plt.xlabel('Time (ms)')
    ...
    plt.axhline(y=avg_diff * 1000, color='b', linestyle='--', alpha=0.5,
                label=f'Avg: {avg_diff*1000:.2f} ms')
    plt.legend()
    ...
    for (ax1, ax2, ax3) in [(plt.subplot(311), plt.subplot(312), plt.subplot(313))]:
        for i, (start, end) in enumerate(key_intervals):
            ...
            ax3.axvspan(start, end, color=colors[i], alpha=alpha)
...
def main():
    ...
    time_slots, bandwidths, intervals, timestamps = calculate_score()

    # 生成图表，根据no-show参数决定是否显示
    plot_metrics(time_slots, bandwidths, intervals, timestamps, show_plot=not args.no_show)
